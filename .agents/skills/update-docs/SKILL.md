...
- 新しい設定UIセクションの追加（例: ポーズ設定、新タブ）
- 既存UIの大幅な変更（レイアウト変更、新しいボタン/トグル追加）
- 新しい画面モードや表示状態（例: ポーズ調整モード、セットアップガイド表示）
- 操作フローの説明に画像があると理解しやすい箇所

#### 確認方法

1. 今回の更新で追加・変更したドキュメントを確認し、スクリーンショットがあるとユーザーの理解を助ける箇所をリストアップする
2. 既存のドキュメント内で画像を使用しているページ（`![...](/images/...)`）を参考に、同等の粒度でスクリーンショットが必要かを判断する
3. 必要なスクリーンショットのリストをユーザーに提示し、撮影・配置を依頼する
